...
class ViewModel: ObservableObject {
    ...
    @Published var source: PickerRunning.Source = .library
    @Published var showCameraAlert = false
    @Published var cameraError: PickerRunning.CameraErrorType?
    ...
    func showPhotoPicker() {
        do {
            if source == .camera {
                try PickerRunning.checkPermissions()
            }
            showPicker = true
        } catch {
            showCameraAlert = true
            cameraError = PickerRunning.CameraErrorType(error: error as! PickerRunning.PickerError)
        }
    }
